...
def init_db():
    ...
    try:
        ...
        c.execute("""CREATE TABLE IF NOT EXISTS attendance (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
                        student_roll_number INTEGER NOT NULL,
                        name TEXT NOT NULL,
                        ...
                        FOREIGN KEY(student_roll_number) REFERENCES students(id)
                    )""")
        c.execute("CREATE INDEX IF NOT EXISTS idx_attendance_date ON attendance(check_in_time)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_attendance_student ON attendance(student_roll_number)")
...
def add_student():
    ...
    return jsonify({"student_roll_number": "temp_" + str(int(time.time() * 1000)), "temp_data": temp_data, "message": "Student info validated. Please upload images."})

@app.route("/upload_face", methods=["POST"])
def upload_face():
    student_roll_number = request.form.get("student_roll_number")
    if not student_roll_number:
        return jsonify({"error":"student_roll_number required"}), 400
    ...
    if os.path.exists(DATASET_DIR) and os.listdir(DATASET_DIR):
        for existing_sid in os.listdir(DATASET_DIR):
            ...
            if not os.path.isdir(existing_folder):
                continue
            if existing_sid == student_roll_number:
                continue
    ...
    is_temp = student_roll_number.startswith("temp_")
    
    temp_folder = os.path.join(DATASET_DIR, student_roll_number)
    ...
    if is_temp and temp_data:
        ...
        actual_student_roll_number = int(student_roll_number)
        conn = get_db()
        try:
            c = conn.cursor()
...
def delete_student(sid):
    try:
        ...
        try:
            ...
            c.execute("DELETE FROM attendance WHERE student_roll_number=?", (sid,))
            conn.commit()
